...
from src.admin.api.poses.schemas import PoseFullOut, PoseOutPagination, PoseIn, OtherTitleIn, OtherTitleInUpdate, \
    OtherTitleOut, ImageIn, PoseInFull
...
@router.put("/{id_pose}")
async def update_pose_full_by_id(id_pose: int, pose_data: PoseInFull, poses_service: PosesService = Depends(PosesService)):
    pose = await poses_service.update_full_pose(id_pose, pose_data)
    return pose


@router.patch("/{id_pose}/info")
...
@router.put("/{id_pose}/other_titles/{id_other_title}")
